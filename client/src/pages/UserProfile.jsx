...
const UserProfile = () => {
  ...
  const loadProfileData = useCallback(async () => {
    try {
      ...
      // console.log('Government ID Proof:', userData.governmentIdProof);
      // console.log('Government ID Type:', userData.governmentIdType);
      setUser(userData);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Profile Header */}
        <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
            {/* User Info */}
            <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto">
              {/* Avatar */}
              <div className="relative flex-shrink-0">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-[#c4ff0d] rounded-full flex items-center justify-center border-2 border-[#1a1a1a]">
                  <span className="text-black text-xl sm:text-2xl font-bold">{user?.name?.charAt(0).toUpperCase()}</span>
                </div>
                <div className="absolute -bottom-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-green-500 rounded-full border-2 border-[#0f0f0f]"></div>
              </div>
              
              {/* Details */}
              <div className="flex-1 min-w-0">
                <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2 truncate">{user?.name}</h1>
                <p className="text-gray-400 text-sm mb-2 sm:mb-3 truncate">{user?.email}</p>
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                  <span className="inline-flex items-center gap-2 px-2 sm:px-3 py-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-xs text-gray-400">
                    <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                    </svg>
                    <span className="truncate">{user?.anonymousId}</span>
                  </span>
                  <span className="inline-flex text-black items-center gap-2 px-2 sm:px-3 py-1 bg-[#c4ff0d] bg-opacity-20 border border-[#c4ff0d] rounded-lg text-xs text-[#c4ff0d] font-semibold">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {/* Phones Listed */}
            <div className="bg-[#1a1a1a] border-2 border-[#2a2a2a] rounded-lg p-3 sm:p-4 hover:border-[#c4ff0d] hover:shadow-lg hover:shadow-[#c4ff0d]/10 transition">
              ...
              <p className="text-xs text-gray-400">Phones Listed</p>
            </div>

            {/* Active Bids */}
            <div className="bg-[#1a1a1a] border-2 border-[#2a2a2a] rounded-lg p-3 sm:p-4 hover:border-[#c4ff0d] hover:shadow-lg hover:shadow-[#c4ff0d]/10 transition relative">
            ...
            <div className="bg-[#1a1a1a] border-2 border-[#2a2a2a] rounded-lg p-3 sm:p-4 hover:border-[#c4ff0d] hover:shadow-lg hover:shadow-[#c4ff0d]/10 transition">
              <div className="flex items-center justify-between mb-2">
                ...
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#c4ff0d]" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>
              <p className="text-xl sm:text-2xl font-bold text-white mb-1">{stats.won}</p>
            ...
            <div className="bg-[#1a1a1a] border-2 border-[#2a2a2a] rounded-lg p-3 sm:p-4 hover:border-[#c4ff0d] hover:shadow-lg hover:shadow-[#c4ff0d]/10 transition">
              ...
              <p className="text-xl sm:text-2xl font-bold text-white mb-1">{stats.reels}</p>
              <p className="text-xs text-gray-400">My Reels</p>
            </div>
          </div>
        </div>
        ...
        {user?.governmentIdType && (
          <div className="bg-[#0f0f0f] border-2 border-[#c4ff0d] rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:justify-between">
              <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#1a1a1a] border border-[#c4ff0d] rounded-lg flex items-center justify-center flex-shrink-0">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#c4ff0d]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-bold text-base sm:text-lg mb-1">Government ID Verification</h3>
                  <p className="text-sm text-gray-400">
                    Status: <span className="text-[#c4ff0d] font-semibold">✓ Verified: {user.governmentIdType || 'Aadhaar'}</span>
                  </p>
                  {!user.governmentIdProof && (
                    <p className="text-xs text-red-400 mt-1">Document not uploaded</p>
                  )}
                </div>
              </div>
              <div className="w-full sm:w-auto">
                {user.governmentIdProof ? (
                  <button
                    onClick={() => {
                      console.log('Opening government ID proof modal:', user.governmentIdProof);
                      setShowIdModal(true);
                    }}
                    className="w-full sm:w-auto px-3 sm:px-4 py-2 bg-[#1a1a1a] border border-[#2a2a2a] text-gray-300 rounded-lg hover:bg-[#2a2a2a] hover:text-white transition text-sm flex items-center justify-center gap-2"
                  >
                  ...
                  <div className="w-full sm:w-auto px-3 sm:px-4 py-2 bg-[#2a2a2a] border border-[#3a3a3a] text-gray-500 rounded-lg text-sm text-center">
        ...
        <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl overflow-hidden">
          {/* Tab Headers */}
          <div className="flex border-b border-[#2a2a2a] overflow-x-auto scrollbar-hide">
            <button
              onClick={() => setActiveTab('selling')}
              className={`flex-shrink-0 whitespace-nowrap py-3 sm:py-4 px-4 sm:px-6 text-center font-semibold transition relative ${
                activeTab === 'selling'
            ...
            >
              <div className="flex items-center justify-center gap-1 sm:gap-2">
                ...
                <span className="ml-1 px-1.5 sm:px-2 py-0.5 bg-[#c4ff0d] text-black rounded-full text-xs font-bold">
                  {stats.phones}
                </span>
              </div>
              {activeTab === 'selling' && (
            ...
            >
              <div className="flex items-center justify-center gap-1 sm:gap-2">
                ...
                <span className="text-sm sm:text-base">My Bids</span>
                <span className="ml-1 px-1.5 sm:px-2 py-0.5 bg-[#c4ff0d] text-black rounded-full text-xs font-bold">
                  {stats.bids}
                </span>
              </div>
            ...
            >
              <div className="flex items-center justify-center gap-1 sm:gap-2">
                ...
                <span className="text-sm sm:text-base">My Reels</span>
                <span className="ml-1 px-1.5 sm:px-2 py-0.5 bg-[#c4ff0d] text-black rounded-full text-xs font-bold">
                  {stats.reels}
                </span>
              </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-[#2a2a2a]">
                                  <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                    bid.isWinning ? 'bg-green-500 bg-opacity-20 text-black' : 'bg-gray-700 text-gray-400'
                                  }`}>
      ...
      {/* Government ID Modal */}
      {showIdModal && user?.governmentIdProof && (
        <div 
        ...
        >
          <div 
            className="relative max-w-2xl w-full max-h-[80vh] bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl overflow-hidden shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Button */}
            <button
              onClick={() => setShowIdModal(false)}
              className="absolute top-3 right-3 z-10 w-8 h-8 bg-[#1a1a1a] border border-[#2a2a2a] text-white rounded-full hover:bg-[#2a2a2a] transition flex items-center justify-center"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-[#0f0f0f] border-b border-[#2a2a2a] p-4">
              <h3 className="text-white font-bold text-base flex items-center gap-2 pr-8">
                <svg className="w-5 h-5 text-[#c4ff0d]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                </svg>
                Government ID - {user.government}
              </h3>
            </div>
            
            {/* Modal Content */}
            <div className="p-4 max-h-[60vh] overflow-auto">
              <div className="flex items-center justify-center bg-[#1a1a1a] rounded-xl p-4 min-h-[300px]">
                <img
            ...
            {/* Modal Footer */}
            <div className="bg-[#0f0f0f] border-t border-[#2a2a2a] p-4 flex justify-between items-center">
              <p className="text-sm text-gray-400">
                Status: <span className="text-[#c4ff0d] font-semibold">✓ Verified</span>
              </p>
              <button
                onClick={() => setShowIdModal(false)}
                className="px-4 py-2 bg-[#c4ff0d] text-black rounded-lg hover:bg-[#d4ff3d] transition font-semibold text-sm"
