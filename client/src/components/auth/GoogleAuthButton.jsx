...
const GoogleAuthButton = () => {
  ...
  // Effect to render button when Google is loaded and ref is available
  useEffect(() => {
    if (googleLoaded && buttonRef.current && !buttonRendered) {
      // Small delay to ensure DOM is ready
      setTimeout(() => {
        renderGoogleButton();
      }, 100);
    }
  }, [googleLoaded, buttonRendered]);
  ...
  const renderGoogleButton = () => {
    if (buttonRef.current && !buttonRendered) {
      try {
  ...
  const handleGoogleLogin = () => {
    ...
    // This function is mainly for fallback - the actual login will be handled by Google's button
